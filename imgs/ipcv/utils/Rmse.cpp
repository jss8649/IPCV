...
double Rmse(const cv::Mat& src1, const cv::Mat& src2) {
  if ((src1.rows != src2.rows) || (src1.cols != src2.cols) ||
    ...
    cerr << "Image dimensions must match exactly for RMSE computation" << endl;
    exit(EXIT_FAILURE);
  }
...
//  mse /= src1.rows * src1.cols * src1.channels();
  ...
  // Compute the root of the mean of the sum of the squared errors
