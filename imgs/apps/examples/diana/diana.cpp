...
class OneTimePad {
  ...
  public:
    // Constructor
    // key
    //   int that specifies the seed for the random number generator
    // rows
    //   int that specifies the number of rows in the one-time pad 
    //   cryto-table
    // words_per_row
    //   int that specifies the number of words in each row of the 
    //   one-time pad crypto-table
    // characters_per_word
    //   int that specifies the number of characters in each word of 
    //   the one-time pad crypto-table
    OneTimePad(int key, int rows, int words_per_row, int characters_per_word) {
      key_ = key;
      rows_ = rows;
      ...
      srand(key_);
      for (int i = 0; i < rows_ * words_per_row_ * characters_per_word_; i++) {
    ...
    int find(string needle) {
      return one_time_pad_.find(needle);
    }
...
int main(int argc, char* argv[]) {
  ...
  int one_time_pad_word_pair_position =
  ...
  int one_time_pad_character_position = one_time_pad_word_pair_position + 
                                        one_time_pad_word_pair.length();
